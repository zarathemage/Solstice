...
class OchreCube(Slab):
    ...
    def collidexy(self, player, tilemap):
        ...
        if type(player.lastmove).__name__ == 'LeftMove': ### left up
            if player.y > self.y+tilemap.tileheight and player.x < self.x+tilemap.tilewidth and self.rightDiagonalSolution(self.y+28,self.y+tilemap.tileheight/2,
                                    self.x+tilemap.tilewidth/2,self.x+tilemap.tilewidth,
                                    player.x, 
                                    player.y):
                return True
        if type(player.lastmove).__name__ == 'DownMove': ### left down
            if player.y < self.y+tilemap.tileheight/2 and player.x < self.x+tilemap.tilewidth and self.rightDiagonalSolution(self.y+28,self.y+tilemap.tileheight/2,
                                    self.x+tilemap.tilewidth/2,self.x,
                                    player.x, 
                                    player.y+player.height):
                return True
        if type(player.lastmove).__name__ == 'RightMove': ### right down
            if player.y > self.y+tilemap.tileheight and player.x > self.x+tilemap.tilewidth and self.leftDiagonalSolution(self.y+28,self.y+tilemap.tileheight/2,
                                    self.x+tilemap.tilewidth/2,self.x+tilemap.tilewidth,
                                    player.x+player.width, 
                                    player.y+player.height):
                return True
        if type(player.lastmove).__name__ == 'UpMove': ### right up
            if player.y > self.y+tilemap.tileheight and player.x > self.x and self.leftDiagonalSolution(self.y+28,self.y+tilemap.tileheight/2,
